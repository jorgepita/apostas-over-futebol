...
**Update (2026-08-02, Phase 27.4) — football-data.org removed entirely; API-Football is now the sole result provider.** A preceding read-only audit (prompted by an unrelated finding — an exposed API key that needed rotating) established that football-data.org, while genuinely used as the first-attempted provider for 6 of the then-22 leagues, was never architecturally required: every league already had a working `af_id`, and the existing per-row fallback logic already proved daily that API-Football alone could settle every league football-data.org didn't cover (16 of 22). The two football-data.org-specific hard startup gates (`if not API_TOKEN: raise ...` in both `main()` and `run_settlement_remote()`) were the only thing actually forcing the key's presence — the settlement *logic* itself never depended on a successful football-data.org call.

Removed entirely: `http_get_json_football_data()`, `fetch_matches_for_league_date()`, `_respect_fd_api_spacing()`, the `fd_matches_cache`/`blocked_fd_leagues_seen` shared-state entries, `classify_fd_status()`, `should_use_api_football_fallback()`, and every `FD_*` constant (rate-limit timing, status sets, void-reason map). `src/league_registry.py`'s `LeagueEntry` lost its `fd_code`/`fd_blocked` fields — replaced by a single `code` field holding the exact same routing-code *value* every league already had (several EU leagues keep their old football-data.org-derived short code — e.g. `"PL"` for Premier League — purely as an opaque, historical identifier string, not because it means anything provider-specific anymore). `BLOCKED_FOOTBALL_DATA_CODES` was deleted outright (nothing left to be blocked from); `API_FOOTBALL_FALLBACK_COMPETITIONS` was renamed to `API_FOOTBALL_COMPETITIONS` (it is the *only* provider mapping now, not a fallback). Settlement routing simplified from a three-way branch (`API-Football direct` / `football-data.org` / `API-Football fallback-after-FD-failure`) to one unconditional path: every eligible row calls the same, previously-fallback-only `_run_af_and_account()` closure inside `update_dataframe()` — `update_dataframe()` itself was **not** split or restructured, per this phase's explicit constraint; only the provider-selection branch inside it was removed. `API_FOOTBALL_KEY`'s own startup check was promoted from a soft warning to the same hard gate `FOOTBALL_DATA_API_KEY` used to have, since API-Football is now the one thing settlement genuinely cannot run without.

Deliberately left untouched: the generic, multi-provider-shape helper functions (`get_fixture_status()`, `get_fixture_score()`, `get_fixture_kickoff_dt()`, `_extract_message_from_http_error_body()`) still contain a dormant football-data.org-shaped fallback branch (checking `fixture.get("utcDate")`/`fixture.get("score")` alongside API-Football's nested shape) — these are generic, actively-used-by-API-Football dual-shape parsers, not football-data.org-specific code, and touching correctness-critical shared parsing logic for a purely cosmetic removal was judged out of scope for "remove football-data.org, do not redesign the settlement engine." They are harmless, unreachable in practice, and documented as such at their call site.

**Update (2026-08-03, Phase 28.2) — three new leagues added; a fourth requested was found already integrated.** Requested: Switzerland Super League, Spain LaLiga Hypermotion (Segunda División), France Ligue 2, Portugal Liga Portugal 2 "Meu Super". A discovery pass (live queries against API-Football's `/leagues` endpoint, 2026-08-03) found **Ligue 2 already a fully active, first-class league** — `src/league_registry.py`'s `franca2` entry (`af_id=62`), registered in `config.json`'s `leagues`/`api_football.league_ids`, with a 549-row `data_raw/franca2.csv` — added previously and never reported as an open gap. A targeted audit (dashboard `LEAGUE_NORMALIZE`, filter dropdowns, analytics, backups, standings) found no hidden gap; re-adding it was rejected as a duplicate-source-of-truth violation of this ADR. Only the three genuinely new leagues were added: `suica` (Switzerland Super League, `af_id=207`), `espanha2` (Spain Segunda División, `af_id=141`), `portugal2` (Portugal Liga Portugal 2, `af_id=95`) — all `"european"` season model, confirmed against live season-date coverage. Display names use each competition's stable, sponsor-free name (matching this project's existing convention — e.g. `"LaLiga"` not `"LaLiga EA Sports"`), not the current commercial names volunteered in the request (`"LaLiga Hypermotion"`, `"Liga Portugal 2 Meu Super"`) — confirmed with the user before implementation.

The audit also surfaced two functional touch points this ADR's own "Adding a new league" checklist (`04_Backend.md`) had never documented, despite every existing league already having them populated: `fetch_oddsapi_fixtures.py`'s `DEFAULT_LEAGUE_IDS` dict (a redundant fallback used only if `config.json` is missing an entry) and, for a league bootstrapped so close to its season's kickoff that the "current" season has near-zero finished matches, an explicit `config.json["historical"]["seasons_by_league"]` override (without it, `fetch_historical.py`'s default month-based season selection would have produced an almost-empty `data_raw` file). Both are now part of the documented checklist. A third dict, `LEAGUE_INFO_EXT` (feeds a fuzzy-search fallback already confirmed dead/unreachable in production per this ADR's Phase 26.42 update), was deliberately **not** populated — EU leagues, including the pre-existing `franca2`, were never in it either; adding entries to genuinely dead code was judged unnecessary.

`data_raw/{suica,espanha2,portugal2}.csv` were built via `fetch_historical.py` (extended with three new `LEAGUE_INFO` entries, matching the Phase 26.42/MLS Next Pro precedent) against real, live API-Football data — seasons 2024 and 2025 (the two most recently completed full seasons), 459/935/615 rows respectively. `data_raw/*.csv` is gitignored by pattern (existing league files are tracked exceptions); the three new files were added with `git add -f`, matching precedent. Three cosmetic, debug-only "EU leagues" set literals (`main.py` ×2, `src/market_rules.py`, `src/pick_generation.py` — each gates only trace-log verbosity, never real filtering or staking logic, confirmed by reading every call site) were extended for consistency, though leaving them out would have caused zero functional difference. `index.html`'s `LEAGUE_NORMALIZE` map gained canonical and display-name entries for the dashboard; filter dropdowns needed no change, already populated dynamically from data rather than a hardcoded list.
...
**Status classification matrix** (API-Football statuses only as of Phase 27.4's ADR-004 update — football-data.org was removed entirely; this table originally also listed its equivalent statuses, e.g. `FINISHED`/`POSTPONED`/`CANCELLED`/`SUSPENDED`, which no longer exist anywhere in the codebase)

| Status | Meaning | Auto-void eligible? | Threshold | Rediscovery? |
|---|---|---|---|---|
| `FT`/`AET`/`PEN` | Finished | N/A — settles normally | — | No |
| `1H`/`HT`/`2H`/`ET`/`BT`/`P`/`LIVE` | In progress | No, at any age | — | No |
| `PST`/`CANC`/`ABD` | Non-played, reasonable finality | Yes | 48h since original kickoff | No |
| `SUSP`/`INT` | Suspended/interrupted, commonly resumes | **No — not via the explicit-status pathway, at any age** | — | — |
| `NS`/`TBD`; anything unrecognised | Scheduled/unknown | No | — | — |
...
1. **SUSP/INT/SUSPENDED reclassified.** The original decision (see the struck-through reasoning this replaces) grouped `SUSP`/`INT`/`SUSPENDED` with `PST`/`CANC`/`ABD` under the same 48h timeout, reasoning that "a suspended or interrupted match needs the identical don't-stay-exposed-forever treatment a postponed one does." The audit's Monday-interrupted/Friday-resumes scenario proved this wrong — it would have auto-voided a wager whose match was still going to finish. `AF_NON_PLAYED_STATUS` in `update_results.py` no longer includes `SUSP`/`INT`; they fall through to the pre-existing `SCHEDULED_UNKNOWN` default (documented via `AF_SUSPENDED_INTERRUPTED_STATUS`, kept for clarity only — not consulted by `classify_af_status()`), reusing the existing safe bucket rather than inventing a fourth classification value or a second timeout. (The equivalent `FD_*` sets and `classify_fd_status()` this paragraph originally also named no longer exist at all — removed entirely along with football-data.org in Phase 27.4, see ADR-004's update.)
