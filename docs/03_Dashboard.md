...
**Dual-view layout (Phase 29.3, positioning refined Phase 29.3B).** The page reads top-to-bottom as four blocks — no tabs, all perspectives always visible:
1. **Visão Geral (Overview)** — `renderAnalyticsOverview()`, a new function reading `{ allTime, currentSeason }` directly (no new aggregation call of its own). Two side-by-side cards (mirroring `renderVersus()`'s existing `sourceCard()`/`kpiCell()` visual pattern, laid out with `grid-template-columns:repeat(auto-fit, minmax(130px, 1fr))` rather than the shared `.kpi-grid-4` class so 7 KPI cells reflow evenly instead of forcing an uneven 4+3 split — the shared class itself is untouched, so `renderVersus()`/the Close Season wizard are unaffected) show Settled Bets, Profit, ROI, Yield (a plain `roi/100` unit conversion, this project's existing convention — see Opinion Validation — not a new calculation), Hit Rate, Average Odds, and Maximum Drawdown for Época Atual vs Histórico Completo at a glance.
2. **Época Atual (Current Season)** — the real-bet-history League/Market/Source performance tables (`renderAnalytics()`'s `analyticsMarketRows`/`analyticsSourceRows`/`analyticsLeagueRows`) and Top/Worst Performers plus this tab's own streak cards (`renderAnalyticsPerformers()`, `#streakCardsAnalytics`) — sourced from `currentSeason` (re-pointed from `allTime` in Phase 29.3).
3. **Desempenho ao Longo do Tempo (neutral transition, Phase 29.3B).** Sits between the Época Atual and Histórico Completo blocks, styled with a muted (not accent/ok-colored) header and an explicit caption ("janelas móveis... independentes da época atual ou de qualquer arquivo") so it reads as neither current-season nor all-time — because its own rolling 7/30/90-day windows are a different, orthogonal dimension to the season boundary, and one of its rows ("Todo o tempo") is genuinely the full unbounded history. `renderPerformanceOverTime()` itself is unchanged — only its section's position and label moved.
4. **Histórico Completo (All-Time Model Analytics)** — League Analytics (the `league_stats.csv`-driven table, unavoidably all-time), League Classification, Market Intelligence, Edge Validation, Strategy Validation, Model Calibration, Action Engine, and Learning Center — all unchanged, still sourced from `allTime`, since these are model-quality metrics that specifically benefit from the largest available sample.

**`renderStreaks()` (`#streakCards`) was deliberately left untouched, still `allTime`** — that element physically lives on the Dashboard Home page (`tab-summary`), not this tab; only Analytics' own `#streakCardsAnalytics` (inside `renderAnalyticsPerformers()`) was re-sourced to `currentSeason`. **`buildAnalyticsDataset()`'s `summary` gained two additive fields** (`avgOdds`, `maxDrawdown`) purely to power the new Overview cards — both computed by reusing existing pure helpers (`avg()`, `computeDrawdownAnalysis()`) over the same `closedRows` the function already receives; no new dataset, no restructuring of any existing field. See `08_Change_Log.md` Phase 29.3/29.3B for the full before/after.

**Responsive layout (Phase 29.4, CSS-only).** Phase 29.3D found (and left out of scope) a pre-existing horizontal-overflow bug affecting the whole Analytics tab at 768–1200px viewport widths, predating the entire Phase 29.x workstream. Root cause: `.section-stack` is `display:grid` with a single implicit column, and CSS grid items default to `min-width:auto` — so a wide, non-wrapping table's intrinsic content size (most visibly `.league-analytics-table`'s own `min-width:980px`) propagated upward through `.card`/`.grid` wrappers, forcing that shared column — and therefore the whole page — wider than the viewport. Fixed with `#tab-analytics .section-stack > *, #tab-analytics .grid > * { min-width: 0; }`, scoped to this tab only; every other page's `.card`/`.grid`/`.section-stack` are untouched. Separately, the pre-existing "Excel-like" sticky-column system (`.table-wrap th/td:nth-child(1-4)`, used by every Analytics table) reserved 90px/110px/180px for columns 2–4 — tuned for wider content elsewhere in the app — while most Analytics tables put short 1–3 digit counts there ("Picks"/"Apostas", and — in the three core League/Market/Source performance tables specifically — "W"/"L"). Tightened via `#tab-analytics`-scoped overrides (with sticky `left` offsets recomputed to match) and a matching reduction of the generic `table { min-width: 840px; }` floor for Analytics' simpler tables (`.league-analytics-table` explicitly excluded — it keeps its own wider floor, since 12 columns genuinely need it). Every Analytics table where every column but the first is a numeric statistic also gained a blanket right-align; tables mixing numeric columns with action badges or explanatory text (Action Engine, League Analytics) were right-aligned column-by-column instead, so badges/free text are unaffected. Verified: the three core performance tables now fit entirely within their card with zero internal scrolling at every tested width (768–1920px); page-level horizontal overflow eliminated end-to-end. See `08_Change_Log.md` Phase 29.4.
...
**Data sources:** `state.leagueStats` (from `league_stats.csv`) enriched with actual bet history via `getAnalyticsAggregates().allTime.byLeague` (Phase 29.2; previously an independent `buildAnalytics(rows, 'league')` call). The enrichment joins by league name and adds real win rates and P&L from executed picks. Displayed under **Histórico Completo** (Section C, as of Phase 29.3B's relettering) — `league_stats.csv` has no season concept in the backend.
