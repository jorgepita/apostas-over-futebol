...
| Phase | Date | Summary |
|---|---|---|
| 29.6 | 2026-08-23 | Code-only fix closing the architectural gap the Phase 29.5/29.5B data-loss incident exposed: `_reloadManualBetsFromCloud()` (Phase 26.17/LIVE-1, called unconditionally on every boot and every tab `visibilitychange`) had no equivalent of `_doLoadCloudState()`'s Phase 28.3A season-recency guard, so a stale (previous-season) cloud snapshot could silently replace `state.manualBets`, persist to `localStorage`, and then be pushed back to GitHub by an ordinary auto-save — exactly the chain that lost 176 current-season manual bets on 2026-08-22. Extracted the existing `_doLoadCloudState()` guard's comparison into one shared helper, `isCloudSessionStale()`, now reused by both cloud-content consumers instead of duplicated/absent. A stale snapshot is rejected outright (nothing mutated) while still marking the sync complete, so Live Center/`saveCloudState()` aren't left waiting indefinitely. 8-scenario real-browser (Playwright/Chromium) regression suite, confirmed to fail against the pre-fix code and pass against the fix (negative control). Full Python suite 430/430 and QuantEngine golden vectors 285/285 unchanged. Zero production data files touched. See `05_Known_Issues.md` SYNC-3 |
| 29.5B | 2026-08-23 | Data recovery: restored 176 current-season manual bets (2026-08-07→08-22 — 95 settled/46W/49L/€95 stake/−€9.38 profit, 80 rejected, 1 pending) missing from `cloud_state.json`, lost via the same 2026-08-22 backup-restore incident as Phase 29.5, but through a separate mechanism (`_reloadManualBetsFromCloud()` — see Phase 29.6). Recovered from the last known-good pre-restore commit (`e43932be`) by a non-destructive union on each bet's unique `id` — 0/176 id overlap with production, so nothing existing was altered; the 216 stale old-season records the restore had also reintroduced were deliberately left untouched (separate, explicitly out-of-scope cleanup). One recovered record (a pending €10,000 "Espanyol vs Real Madrid" bet) was investigated (Phase 29.5C) and found to be a genuine-provenance but likely erroneous stake entry — preserved as data, later removed by the user through the normal dashboard UI. Full Python suite 430/430 unchanged (data-only, `cloud_state.json`). See `05_Known_Issues.md` SYNC-3 |
| 29.5 | 2026-08-23 | Data recovery: restored 161 bot-pick rows (140 settled, 2026-08-07→08-22) missing from `picks_history.csv`/`league_stats.csv` after a manual Backups-tab "Restore" action reverted them to a 2026-08-02 snapshot — initially investigated as a suspected History season-boundary regression (`isOnOrAfterSession()`/`getFilteredRealClosedRows()`/`getHistoryFilteredRows()` were all confirmed correct and unchanged since Phase 28.5), but traced to this data-loss incident instead. Recovered from the last known-good pre-restore commit (`24c475c2`) via a per-row-key union (production wins on any key conflict) — 0 rows removed/overwritten from production, `league_stats.csv` recomputed from the corrected history via the project's own `update_league_stats()`. Full Python suite 430/430 unchanged (data-only). See `05_Known_Issues.md` SYNC-3 |
| 29.4 | 2026-08-06 | CSS-only responsive layout refinement, closing the pre-existing Analytics horizontal-overflow debt (768–1200px) Phase 29.3D found and explicitly deferred, plus table column-width/alignment tightening. Root cause: `.section-stack`'s single implicit grid column shares one track width across every child; grid items default to `min-width:auto`, so a wide table's own intrinsic size (`.league-analytics-table`'s `min-width:980px`) propagated upward through `.card`/`.grid` wrappers, forcing the shared column — and the whole page — wider than the viewport (confirmed reproducible on the pre-Phase-29.2 file). Fixed with one scoped rule, `#tab-analytics .section-stack > *, #tab-analytics .grid > * { min-width: 0; }`. Separately, the pre-existing sticky-column system's 90px/110px/180px minimums for table columns 2–4 (tuned for wider content elsewhere) left far more room than most Analytics tables' short numeric columns ("Picks"/"Apostas", and — in the three core League/Market/Source tables — "W"/"L") need; tightened via `#tab-analytics`-scoped overrides (sticky `left` offsets recomputed) plus a matching reduction of the generic `table { min-width: 840px; }` floor for Analytics' simpler tables (`.league-analytics-table` excluded, keeps its own wider floor). Every table where every column but the first is numeric gained a blanket right-align; tables mixing numbers with badges/text (Action Engine, League Analytics) were right-aligned column-by-column instead. Verified: `git diff` — 107 lines added, 0 removed, entirely inside `<style>`; stripped-text content of a full render byte-for-byte identical before/after; zero horizontal overflow at 768/850/1024/1200/1440/1920px with a full realistic dataset (was overflowing at every width 769–1200px before); the three core tables now fit entirely within their card with no internal scrolling at any tested width. Full Python suite 430/430 and QuantEngine golden vectors 285/285 unchanged. See `03_Dashboard.md` §6 |
| 29.3B | 2026-08-04 | Final UX polish on the Analytics dual-view UI, resolving the Phase 29.3A audit's two non-blocking findings — deployment preparation only, zero aggregation/calculation/data-source change. Relocated Performance Over Time out of the Época Atual group into its own neutral, muted-colored transition section between Época Atual and Histórico Completo, with an explicit caption clarifying its rolling 7/30/90-day windows (and "Todo o tempo" row) are independent of the season boundary; `renderPerformanceOverTime()` itself and its data source are unchanged — confirmed via `git diff` showing zero lines touched inside any JS function body. Histórico Completo's sections relettered D–J (was D–K) to close the gap left behind. Analytics Overview's 7-KPI grid switched from the shared `.kpi-grid-4` class (forced 4 columns, uneven 4+3 split) to a scoped inline `grid-template-columns:repeat(auto-fit, minmax(130px, 1fr))` — visually confirmed via screenshot to reflow into a balanced layout, fluid (no fixed pixel widths), isolated to this one component so `renderVersus()`'s identical-pattern card and the Close Season wizard are unaffected. Verified: all 16 element IDs unchanged; the 7 All-Time sections plus the `league_stats.csv` table still byte-for-byte identical to the Phase 29.2 deployment baseline; full Python suite 430/430 and QuantEngine golden vectors 285/285 unchanged. See `03_Dashboard.md` §6 |
...
## Phase 29.4 — Analytics Responsive Layout Refinement

**Implemented:** 2026-08-06

**Background:** Phase 29.3D found the deployed Analytics tab overflowed horizontally at 768–1200px viewport widths, and proved — by testing the original pre-Phase-29.2 `index.html` at the same widths and reproducing identical overflow numbers — that this predated the entire Phase 29.x workstream and was not caused by the Overview grid it was scoped to fix. That finding was deliberately left unaddressed (out of scope) in Phase 29.3D/29.3E's deployment. This phase closes it, CSS/layout-only, with no aggregation, calculation, filtering, memoization, or renderer change permitted or made.

**Root cause (Task 1), confirmed via direct DOM measurement, not assumption:** `.section-stack` is `display:grid` with no `grid-template-columns` — exactly one implicit column, whose width is shared by every child. CSS grid items default to `min-width:auto`, which resolves to the item's own min-content size unless overridden. `.card` (or, for two-cards-side-by-side sections, the intervening `.grid.grid-2` wrapper) is that grid item; when it contains a wide, non-wrapping table — most visibly `.league-analytics-table`'s own explicit `min-width:980px` — its automatic minimum size grows to match, even though `.table-wrap`/`.league-analytics-scroll` already correctly scroll that overflow internally (`overflow:auto`). Measured directly: at 1024px, `.section-stack` itself was 992px (correct), but its child `.card` was 1090px (blown out beyond its own parent). Confirmed this is not new: the original pre-Phase-29.2 file reproduces the exact same overflow (1106px at 850/1024px, 1298px at 1200px) with no Overview code present at all.

**Fix (`index.html`, CSS only, 107 lines added, 0 removed, all inside `<style>`):**
- **Task 1 (root cause):** `#tab-analytics .section-stack > *, #tab-analytics .grid > * { min-width: 0; }` — scoped to this tab only; every other page's `.card`/`.grid`/`.section-stack` (History, Bank, Bot vs Manual, Strategy Lab, ...) are untouched. Verified: zero horizontal overflow at every one of 13 tested widths from 768px to 1920px, with both a minimal and a full realistic dataset.
- **Task 2 (table column optimisation):** the pre-existing "Excel-like" sticky-column system (`.table-wrap th/td:nth-child(1-4)`, `position:sticky`) reserves 90px/110px/180px minimums for columns 2–4 — tuned for wider content elsewhere in the app (History, etc.). Most Analytics tables put a short 1–3-digit "Picks"/"Apostas" count at column 2 (universal, tightened to 56px, with column 3's sticky `left` offset recomputed to match); the three core League/Market/Source performance tables (`analyticsMarketRows`/`analyticsSourceRows`/`analyticsLeagueRows`) additionally put single-letter result counts ("W"/"L") at columns 3–4 (tightened to 38px each, `left` offsets recomputed), targeted by their own table-body IDs rather than a blanket rule that would have wrongly shrunk other tables' differently-shaped columns 3–4 (e.g. "Taxa de Acerto" in most other tables). The generic `table { min-width: 840px; }` floor — which was silently absorbing every column-width reduction above by redistributing the same fixed 840px total across the remaining columns instead of letting the table shrink — is reduced to `min-width: 0` for Analytics' simpler tables specifically (`table:not(.league-analytics-table)`, scoped to `#tab-analytics`); `.league-analytics-table`'s own separate, more specific `min-width: 980px` rule is untouched, since its 12 columns genuinely need the room.
- **Task 3 (numeric alignment):** every Analytics table where every column but the first (a league/market/period/edge-range label) is itself numeric gets a blanket right-align (Market/Source/League performance, Performance Over Time, Market Intelligence, Edge Validation, all 6 Strategy Validation tables, both Model Calibration tables). `analyticsMarketRows`/`SourceRows`/`LeagueRows` are IDs on the `<tbody>`, not the `<table>`, so a first attempt using `#analyticsMarketRows th` silently matched nothing (no `<th>` exists inside a `<tbody>`) — caught by directly inspecting `getComputedStyle()` rather than trusting the selector; fixed with `table:has(#analyticsMarketRows) th` (`:has()` was already an established pattern in this file's tier-background rules). Tables mixing numeric columns with action badges or explanatory text (Action Engine's 4 tables — "Ação"/"Motivo"; the League Analytics 12-column table — "Ação"/"Confiança"/"Stake Dinâmica"/"Nível") were right-aligned column-by-column instead (verified against a live column-header survey of all 19 distinct table shapes in the tab), leaving every badge and free-text column exactly as it was.

**Verification:**
- `git diff index.html`: 107 lines added, 0 removed, every changed line inside the `<style>` block. No JS function body touched.
- Stripped-text content (all HTML tags removed) of a full Analytics render, same synthetic dataset, captured before and after this phase's changes: **byte-for-byte identical** — confirms zero metric, data, or calculation change.
- `node --check`: OK. Full Python suite: 430/430 passing, unchanged. QuantEngine golden vectors: 285/285, unchanged.
- Horizontal-overflow scan at 768/800/850/900/1000/1024/1100/1200/1300/1400/1440/1600/1920px, both with a minimal and a full realistic dataset: **zero overflow at every width** (was overflowing at every width from 769px to 1200px before this phase).
- Screenshots (scratchpad-only) confirm: all 9 columns of the Market/Source/League performance tables visible with no internal scrollbar at 1024px (previously only 4 of 9 columns were visible, with the remainder requiring a hidden internal scroll); numeric columns visibly right-aligned; League Analytics' 12-column table correctly right-aligns only its 6 numeric columns (ROI%, Taxa de Acerto, Kelly Médio, Picks, Stake€, P/L€) while its 6 badge/text columns (Liga, Mercado, Ação, Confiança, Stake Dinâmica, Nível) remain untouched; Model Calibration's table right-aligns correctly with its sample-quality badges intact.

**Scope:** `index.html` `<style>` block only. No calculation, aggregation, filtering, memoization, renderer, or non-Analytics change. No new committed test file, consistent with this project's established scratchpad-only Playwright convention.

---

## Phase 29.3B — Analytics Final UX Polish & Deployment Preparation

**Implemented:** 2026-08-04

**Background:** Phase 29.3A (read-only audit) concluded **PASS** for Phase 29.3's dual-view implementation, with two non-blocking findings: (1) Performance Over Time sat inside the Época Atual group while its own rolling windows (including a "Todo o tempo" row) are independent of the season boundary — a real UX ambiguity, not a bug; (2) the Overview's 7-KPI cards rendered as an uneven 4+3 grid. This phase resolves both, presentation-only, with the explicit constraint of touching neither the aggregation layer nor any calculation.

**Task 1 — Performance Over Time repositioned (`index.html` markup only):** moved out of the Época Atual group entirely. It now sits as its own standalone block between Época Atual and Histórico Completo, with a muted-colored (`var(--muted)`, distinct from Época Atual's accent and Histórico Completo's ok-green) section header carrying no letter — signalling it belongs to neither lettered group. `renderPerformanceOverTime()` itself, and every line inside it, is untouched — confirmed via `git diff` showing zero changes inside any JS function body for this task.

**Task 2 — Caption added:** the card's existing note was expanded to explicitly name its scope: *"Apostas reais fechadas (bot + manuais) — janelas móveis (7/30/90 dias e todo o tempo), independentes da época atual ou de qualquer arquivo."* The section's own header title was similarly made explicit: *"Desempenho ao Longo do Tempo — Janelas Móveis (Independente da Época)."*

**Relettering (consequence of Task 1, not separately requested but needed to avoid a dangling gap):** Histórico Completo's 8 sections relettered from D (League Analytics) through K (Learning Center) to C (League Analytics) through J (Learning Center), since Performance Over Time no longer occupies a letter in either sequence. Época Atual's A/B are unaffected. Purely cosmetic text changes to `section-header-title` spans and their matching HTML `<!-- SECTION X -->` comments — no ID, no JS reference, no behaviour affected.

**Task 3 — Overview grid adjusted:** `renderAnalyticsOverview()`'s `seasonCard()` grid changed from `class="kpi-grid kpi-grid-4"` (the shared, fixed-4-column class also used by `renderVersus()`'s `sourceCard()` and the Close Season wizard's summary grid) to `class="kpi-grid"` with a scoped inline `style="grid-template-columns:repeat(auto-fit, minmax(130px, 1fr));"`. This is deliberately self-contained: the shared `.kpi-grid-4` CSS class itself was not touched, so every other consumer of it is byte-for-byte unaffected. Uses `minmax(_, 1fr)` (a fluid floor, not a fixed width) so the grid remains fully responsive. Visually confirmed via a Playwright screenshot: at a typical desktop width, the 7 cells now reflow into a balanced 3+3+1 layout rather than the previous rigid 4+3.

**Verification:**
- `git diff` of `index.html` reviewed in full: every changed line is either a `section-header-title`/HTML-comment text change, the single grid CSS swap, or the caption text — zero lines inside any JS function body besides that one CSS attribute.
- All 16 pre-existing Analytics element IDs (15 from Phase 29.2 + `analyticsOverviewWrap` from Phase 29.3) unchanged; div nesting balanced (124 open/124 close).
- The 7 All-Time `*Wrap` sections plus the `league_stats.csv` table re-confirmed **byte-for-byte identical** to the original Phase 29.2 deployment baseline.
- `currentSeason`/`allTime` figures re-confirmed identical to the Phase 29.3 verification run (20 vs 97 settled picks, same avgOdds/maxDrawdown values) — proving this phase changed no metric.
- `node --check` on the extracted `<script>` block: OK. Full Python suite: 430/430 passing, unchanged. QuantEngine golden vectors: 285/285, unchanged.
- Visual verification via Playwright screenshots: confirmed the Overview's 3+3+1 reflow, and confirmed the neutral Performance Over Time section renders in muted color, positioned correctly between Época Atual's content and the "Histórico Completo" divider, with its caption visible.

**Scope:** `index.html` only (`#tab-analytics` markup + one CSS-attribute change inside `renderAnalyticsOverview()`). No aggregation, memoization, calculation, filtering, or backend change. No new committed test file, consistent with this project's established scratchpad-only Playwright convention.
