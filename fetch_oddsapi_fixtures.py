...
def http_get_json(url: str):
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "modelo-over-system/1.0"
        }
    )
...
def build_url(sport_key: str) -> str:
    if not API_KEY:
        raise SystemExit("Falta ODDS_API_KEY. Define no PowerShell: setx ODDS_API_KEY '...'.")
    params = {
        "apiKey": API_KEY,
        "regions": REGIONS,
        "markets": MARKETS,
...
def main():
    ...
    for league_key, sport_key in SPORTS.items():
        ...
        for ev in data or []:
            ...
            # só entra se tiver os dois mercados
    ...
    if errors:
        print("\nAvisos por liga (se algum sport_key não existir):")
        for e in errors:
            print(" -", e)
